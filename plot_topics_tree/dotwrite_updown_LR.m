function [edgelist,nodelist] = dotwrite_updown_LR(fid,nodeFrom,layer,Phisort,S,edgelist,nodelist,endlayer,MaxSize,rsort,NetWorkOnly,minWeight,Colors)
...
%         end
       ...
       if layer>endlayer
         Str=['"' num2str(layer) '_' num2str(nodeFrom) '"' ' [ rank =' num2str(layer) FontSize ', shape=box, style="rounded", color=' Colors{layer}  ',label=' '"' num2str(nodeFrom) ' ' strjoin(SS(1:2)) '\n' strjoin(SS(3:4)) '\n' strjoin(SS(5:6)) '\n' strjoin(SS(7:8))...
                 '\n' strjoin(SS(9:10)) '\n' strjoin(SS(11:12)) '"' ']\n' ];
...
%                 '\n' strjoin(SS(5))  '\n' strjoin(SS(6))  '\n' strjoin(SS(7))  '\n' strjoin(SS(8)) '\n' strjoin(SS(9)) '\n' strjoin(SS(10)) '\n' strjoin(SS(11)) '\n' strjoin(SS(12))  '"' ']\n' ];    
        ...
        for nodeTo = dex(:)'
            ...
            if layer-1>endlayer
             Str = ['"' num2str(layer-1) '_' num2str(nodeTo) '"' ' [rank =' num2str(layer+1)  FontSize ', shape=box, style="rounded", color=' Colors{layer-1}  ',label=' '"' num2str(nodeTo) ' ' strjoin(SS(1:2)) '\n' strjoin(SS(3:4)) '\n' strjoin(SS(5:6)) '\n' strjoin(SS(7:8))...
                 '\n' strjoin(SS(9:10)) '\n' strjoin(SS(11:12)) '"' ']\n' ];
...
%                 '\n' strjoin(SS(5))  '\n' strjoin(SS(6))  '\n' strjoin(SS(7))  '\n' strjoin(SS(8)) '\n' strjoin(SS(9)) '\n' strjoin(SS(10)) '\n' strjoin(SS(11)) '\n' strjoin(SS(12))  '"' ']\n' ];
            ...
            Str = ['"' num2str(layer-1) '_' num2str(nodeTo) '"' ' [rank =' num2str(layer+1)  FontSize ', shape=box, style="rounded", color=' Colors{layer-1}  ',label=' '"' num2str(nodeTo) ' ' strjoin(SS(1:6)) '\n' strjoin(SS(7:12))  '"' ']\n' ];
            end
            if NetWorkOnly
        ...
        for j_in=dex(:)'
            [edgelist,nodelist]=dotwrite_updown_LR(fid,j_in,layer-1,Phisort,S,edgelist,nodelist,endlayer,MaxSize,rsort,NetWorkOnly,minWeight,Colors);
        end
else
%     nodeTo=nodeFrom;
