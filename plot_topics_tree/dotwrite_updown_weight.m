function [edgelist,nodelist] = dotwrite_updown_weight(fid,nodeFrom,layer,Phisort,S,edgelist,nodelist,endlayer,MaxSize,rsort,NetWorkOnly,minWeight,Colors)
...
if layer>endlayer
        ...
        dex=find(Phisort{layer}(:,nodeFrom)>minWeight(layer)/size(Phisort{layer},1));
%         if layer>2
...
%         end
       ...
       if layer>endlayer
        ...
        for j_in=dex(:)'
            [edgelist,nodelist]=dotwrite_updown_weight(fid,j_in,layer-1,Phisort,S,edgelist,nodelist,endlayer,MaxSize,rsort,NetWorkOnly,minWeight,Colors);
        end
else
%     nodeTo=nodeFrom;
