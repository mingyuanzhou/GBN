...
for row = 1:RowNum
    for col = 1:ColNum
        ...
        if k<=K
            ...
            I( (row-1)*(PatchSize+1)+2 : row*(PatchSize+1) , (col-1)*(PatchSize+1)+2 : col*(PatchSize+1), 1) = 1;
            I( (row-1)*(PatchSize+1)+2 : row*(PatchSize+1) , (col-1)*(PatchSize+1)+2 : col*(PatchSize+1), 2) = 1;
            I( (row-1)*(PatchSize+1)+2 : row*(PatchSize+1) , (col-1)*(PatchSize+1)+2 : col*(PatchSize+1), 3) = 1;
        end
    end
end
imshow(I,[]);
