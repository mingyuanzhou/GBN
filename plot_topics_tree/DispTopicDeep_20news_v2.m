%Reproduce Figures 6-7 and 17-21 of Mingyuan Zhou, Yulai Cong, and Bo Chen, ?Augmentable gamma belief networks," Journal of Machine Learning Research, vol. 17, pp. 1-44, Sept. 2016.

eta0 = 0.1;
K=800
KKK=K;

load(['/Users/zhoum/Box Sync/GBN_results/results1_global/ZGlobalTrim_20newsBydate_K0_',num2str(K),'_T_5_eta0',num2str(eta0*1000),'_PreTrain0_AllForPhi0_TrimStrategy5_Trial1_Data.mat'])
...
%download the topictoolbox from
%http://psiexp.ss.uci.edu/research/programs_data/toolbox.htm and change the
...
%% plot all the trees rooted at a top-layer node
startlayer=5;
endlayer=1;
...
for nodeFrom=1:min(40,size(Phisort{startlayer},2))
    ...
    NetWorkOnly = false;
    %minWeight=[3,2,3,3,3,3,3,3];
    % minWeight=[2.5,2.5,2.5,2.5,2.5,2.5,2.5];
    %minWeight=[2.5,2.5,2.5,2.5,2.5,2.5,2.5];
    minWeight=[3,3,3,3,3,3,3,3,3];
...
%% plot all the trees rooted at a third-layer node 
startlayer=3;
endlayer=1;
...
if 0
    ...
    TopicsAll={'car','god','sports','tax','gun','car_sales','hardware','nasa','security','med','isreal'};
